...
const MenuOption: FunctionComponent<Props> = ({ children, className, type }) => {
    if (type === 'multiple') {
        return (
            <div className={classNames('editor-floating-menu-dropdown__menu-option', className)}>
                {children}
            </div>
        );
    }
    ...
    if (type === 'paragraph') {
        ...
        return (
            <ParagraphElement
                className={classNames('editor-floating-menu-dropdown__menu-option', className)}
