...
export function withFilesPasting<T extends Editor>(editor: T): T {
    const parent = {
        insertData: editor.insertData,
    };

    editor.insertData = (dataTransfer: DataTransfer) => {
        const isFilesOnly = dataTransfer.types.length === 1 && dataTransfer.types[0] === 'Files';
        ...
        files.forEach(async (file) => {
            const uploadedFileInfo = await insertUploadingFile<PrezlyFileInfo>(editor, {
                createElement: (fileInfo) => {
                    ...
                    const attachment = UploadcareFile.createFromUploadcareWidgetPayload(fileInfo);
