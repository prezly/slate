...
function UnicodeRegExp(pattern: string, flags = ''): RegExp {
    if (isUnicodeRegExpSupported()) {
        // Do not expand `\p{L}` tokens for browsers that support it out-of-the-box
        return new RegExp(pattern, flags.includes('u') ? flags : `${flags}u`);
    }

    // This monster contains every official language in the world
    // For problems, blame Ivan & Lukas :)
    const Letter =
    ...
    return new RegExp(
        pattern.replace(/\\p{L}/g, Letter),
        flags.includes('u') ? flags.replace('u', '') : flags,
    );
}

function isUnicodeRegExpSupported(): boolean {
