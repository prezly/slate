...
describe('isOrderedListSymbol', () => {
    it('Recognizes symbols of unordered lists', () => {
        const unorderedListSymbols = ['§', '·', 'o', 'r', 'v', '', 'Ø', 'ü', '¢', '¿'];
        ...
        unorderedListSymbols.forEach((symbol) => {
            ...
            expect(isOrderedListSymbol(`${NO_BREAK_SPACE}${symbol}${NO_BREAK_SPACE}`)).toBe(false);
            expect(isOrderedListSymbol(`${symbol}${NO_BREAK_SPACE}`)).toBe(false);
            expect(isOrderedListSymbol(`${NO_BREAK_SPACE}${symbol}${SPACE}`)).toBe(false);
            expect(isOrderedListSymbol(`${symbol}${SPACE}`)).toBe(false);
            expect(isOrderedListSymbol(`${NO_BREAK_SPACE}${symbol}${NO_BREAK_SPACE}${SPACE}`)).toBe(
                false,
            );
            expect(isOrderedListSymbol(`${symbol}${NO_BREAK_SPACE}${SPACE}`)).toBe(false);
        });
    });
    ...
    it('Recognizes symbols of ordered lists', () => {
        ...
        orderedListSymbols.forEach((symbol) => {
            ...
            expect(isOrderedListSymbol(`${NO_BREAK_SPACE}${symbol}${NO_BREAK_SPACE}`)).toBe(true);
            expect(isOrderedListSymbol(`${symbol}${NO_BREAK_SPACE}`)).toBe(true);
            expect(isOrderedListSymbol(`${NO_BREAK_SPACE}${symbol}${SPACE}`)).toBe(true);
            expect(isOrderedListSymbol(`${symbol}${SPACE}`)).toBe(true);
            expect(isOrderedListSymbol(`${NO_BREAK_SPACE}${symbol}${NO_BREAK_SPACE}${SPACE}`)).toBe(
                true,
            );
            expect(isOrderedListSymbol(`${symbol}${NO_BREAK_SPACE}${SPACE}`)).toBe(true);
        });
    });
});
